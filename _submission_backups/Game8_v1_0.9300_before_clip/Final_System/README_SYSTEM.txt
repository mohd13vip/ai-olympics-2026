...
The run prints a validation leaderboard. The selected system should be:
    ...
    relation_gradient_boosting: 0.9300 <- selected

(verified reproducible on CPU; predictions for the public test match the
submitted game8_public_predictions.csv).
...
- Fusion: 19 features (expert probabilities, confidences, agreement,
  probability gap/product, relation match/mismatch interactions, text
  length/missingness) feed five candidate fusion systems. The one with the
  best validation macro-F1 is selected automatically (expected:
  HistGradientBoostingClassifier stack, refit on train+validation before
  scoring the hidden test).
...
score_cache/               precomputed expert scores for all 13,000
                           reference samples; only images/texts not already
                           in the cache are scored at run time, so a hidden
                           test of N samples costs at most N image + N text
                           inferences (seconds on CPU for hundreds of rows)
