...
Everything the system needs ships inside this folder:
    ...
    score_cache/              precomputed expert scores; only unseen images and
                              texts are scored at run time

No network access is required: the sentence-transformer encoder is loaded from
the local folder, never downloaded.
"""
...
def evidence_fields(
    ...
    mismatch = frame["relation_mismatch"].to_numpy().astype(bool)
    evidence: list[str] = []
...
def main() -> None:
    ...
    image_scores, text_scores = cached_scores(
    ...
    logistic.fit(train_f[FEATURE_COLUMNS], y_train)
    boosted.fit(train_f[FEATURE_COLUMNS], y_train)
